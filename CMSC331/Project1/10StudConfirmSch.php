...
<html lang="en">
  <head>
    <meta charset="UTF-8" />
    <title>Confirm Appointment</title> <!--style referencing-->
...
$firstn = $_SESSION["firstN"]; //inputted information from user
			...
			if($_SESSION["resch"] == true){
			  //if reschedule is selected, it will prompt the user to re-enter the appointment information
				...
				if($oldAdvisorID != 0){
				  //if the advisor wasnt selected, it will prompt the student to select an advisor
			...
			if($currentAdvisorID != 0){
			  //if current advisors is selected, it will allow user to select another advisor
		...
		<?php
			if($_SESSION["resch"] == true){
			  //if rescheudling is selected, it will prompt user to select the rescheudle button

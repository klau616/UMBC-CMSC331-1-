...
<?php
	...
	$COMMON = new Common($debug);
      ...
      $User = $_SESSION["UserN"];
       ...
       // Gathers information from Proj2Advisor by passing in username
	...
	if($_POST["type"] == 'Both')
	{
	  //if user selected to display both appointment type, it will prints out information for both type
		displayGroup($id, $date);
		displayIndividual($id, $date);
	}
elseif($_POST["type"] == 'Individual') { displayIndividual($id, $date); } //if user selceted to display individual only, it would prints out individual appointments
elseif($_POST["type"] == 'Group') { displayGroup($id, $date); } //if user selected to display group only, it would prints out group appoints only
...
<?php
	...
	function displayGroup($id, $date) //this function is use to display the members in the group
{
...
function displayIndividual($id, $date) //this function is use to display individual information from the appointment
{
	global $debug; global $COMMON;
